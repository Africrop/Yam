...
results <- NULL
for(m in c("1","2","3")){
  for( i in list.files(paste0("Wild",m))){
    if(file.exists(paste("./Wild",m,"/",i,"/Wild",m,".bestlhoods",sep=""))){
      results <- rbind(results,
                       cbind(m, i,
			     read.table(paste("./Wild",m,"/",i,"/Wild",m,".bestlhoods",sep=""),
                                             header=TRUE)))
    }
  }
}
...
for(m in c("1","2","3")){
  for( i in list.files(paste0("Wild",m))){
    if(file.exists(paste("./Wild",m,"/",i,"/Wild",m,"_","maxL.lhoods",sep=""))){
      results <- rbind(results,
                       cbind(m, i,
                                  read.table(paste("./Wild",m,"/",i,"/Wild",m,"_","maxL.lhoods",sep=""),
                                             header=TRUE)))
    }
  }
}
