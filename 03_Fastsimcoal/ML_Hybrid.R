...
results <- NULL
for(m in c("1","2","3")){
  for( i in list.files(paste0("Hybrid",m))){
    if(file.exists(paste("./Hybrid",m,"/",i,"/Hybrid",m,".bestlhoods",sep=""))){
      results <- as.data.frame(rbind(results,
                       cbind(m, i,
                                  read.table(paste("./Hybrid",m,"/",i,"/Hybrid",m,".bestlhoods",sep=""),
                                             header=TRUE)$MaxEstLhood)))
    }
  }
}

names(results)=c("m", "i", "MaxEstLhood")


# get the max likelihood for each model
ML <- NULL
for(m in levels(results$m)){
  ML <- rbind(ML, data.frame(m, results[which(results$MaxEstLhood==(max(as.numeric(as.character(results[results$m == m,]$MaxEstLhood))))),]$i,max(as.numeric(as.character(results[results$m == m,]$MaxEstLhood)))))
}
...
for(m in c("1","2","3")){
  for( i in list.files(paste0("Hybrid",m))){
    if(file.exists(paste("./Hybrid",m,"/",i,"/Hybrid",m,"_","maxL.lhoods",sep=""))){
      results <- rbind(results,
                       cbind(m, i,
                                  read.table(paste("./Hybrid",m,"/",i,"/Hybrid",m,"_","maxL.lhoods",sep=""),
                                             header=TRUE)))
    }
  }
}
...
# Assuming xx parameters for each model (here xx=11)
# Caution : convert log10(likelihoods) from fsc to ln(likelihoods) by multiplying by log(10)
ML$AIC <- 2*11 - 2*(ML$max_likelihood*log(10))
